...
_SCANNABLE_EXTENSIONS = {
    ".md", ".txt", ".py", ".sh", ".bash", ".js", ".ts", ".yaml", ".yml",
...
class HermesHarnessAdapter(HarnessAdapter):
    ...
    def _scan_skill(
        self, category_dir: Path, skill_dir: Path, skill_md: Path,
    ) -> list[GuardArtifact]:
        ...
        for subdir_name in _SKILL_SUBDIRS:
            ...
            for file_path in sorted(subdir.rglob("*")):
                ...
                artifacts.append(
                    GuardArtifact(
                        artifact_id=(
                            f"hermes:skill:{category_dir.name}:{skill_dir.name}"
                            f":{rel_path}"
                        ),
    ...
    def _scan_mcp_servers(
        self, hermes_home: Path, found_paths: list[str],
    ) -> list[GuardArtifact]:
        ...
        for name, server_config in servers.items():
            ...
            auth_header_keys = [
                ...
                if isinstance(k, str) and any(
                    t in k.lower() for t in ("auth", "token", "key", "secret", "bearer")
                )
            ]
